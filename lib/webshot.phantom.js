...
optUtils.phantomPage.forEach(function(key) {
  ...
  if (key === "paperSize" && page[key]) {
    page.paperSize = {
      "format": options.paperSize.format,
      "orientation": options.paperSize.orientation,
      "margin": options.paperSize.margin
    }
    if (page[key].header && page[key].header.contents) {
      var header = {
        "height": page[key].header.height,
        "contents": eval(page[key].header.contents)
      }
      page.paperSize.header = header;
    }
    if (page[key].footer && page[key].footer.contents) {
      var footer = {
        "height": page[key].footer.height,
        "contents": eval(page[key].footer.contents)
      }
      page.paperSize.footer = footer
    }
  }
});
